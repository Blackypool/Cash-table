...
#define SKIP_ARGS_IN_TXT ptr_to_base[i] == ' ' || ptr_to_base[i] == '\t' || ptr_to_base[i] == '\n' || ptr_to_base[i] == '"' || ptr_to_base[i] == '*' \
            || ptr_to_base[i] == '(' || ptr_to_base[i] == ',' || ptr_to_base[i] == '.' || ptr_to_base[i] == '?' || ptr_to_base[i] == '!' || ptr_to_base[i] == ')'
...
char** update_bd(size_t *now_word)
{
    ...
    size_t how_much_words = number_char / 2 + 12;
    ...
    for(size_t i = 0; i < number_char; i++)
    {
        ...
        while(i < number_char && !(SKIP_ARGS_IN_TXT))
            i++;
